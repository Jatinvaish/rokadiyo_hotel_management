...
@Injectable()
export class SubscriptionPermissionsGuard implements CanActivate {
  constructor(
    private subscriptionsService: SubscriptionsService
  ) {}
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Get subscription permissions for tenant
    const subscriptionPermissions = await this.subscriptionsService.getSubscriptionPermissions(user.tenantId);

    // Store subscription permissions in request for later use
    request.subscriptionPermissions = subscriptionPermissions.map(p => p.permission_key);
